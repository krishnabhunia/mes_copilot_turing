...
def translate_docx_with_formatting(input_path, output_path):
    ...
    # Translate and copy tables
    for table in doc.tables:
        new_table = translated_doc.add_table(rows=len(table.rows), cols=len(table.columns))
        for i, row in enumerate(table.rows):
            for j, cell in enumerate(row.cells):
                if cell.text.strip():
                    translated_text = translate_text(cell.text, tokenizer, model)
                else:
                    translated_text = ""
                new_table.cell(i, j).text = translated_text
                new_table.cell(i, j).paragraphs[0].style = cell.paragraphs[0].style  # Retain cell style
